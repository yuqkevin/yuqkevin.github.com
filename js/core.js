...
(function($) {
	...
	$.fn.center = function(options) {
		var conf = {
			...
			'topOffset': 0,		// adjustment
			'leftOffset': 0
		}
		if (options) $.extend(conf, options);
		var parent = conf.parent=='window'?$(window):this.parent();
	    this.css("position","absolute");
    	this.css("top", Math.max(0, conf.topOffset+((parent.height() - this.outerHeight()) / 2) + parent.scrollTop()) + "px");
	    this.css("left", Math.max(0, conf.leftOffset+((parent.width() - this.outerWidth()) / 2) + parent.scrollLeft()) + "px");
    	return this;
	};
